...
void cycle_report_coord(void){
    // Live commanded position from GRBL's stepper-ISR-maintained step counts.
    // No I²C encoder reads — those block interrupts and starve the stepper
    // ISR, causing visible motion jitter at low speeds.
    //
    // Joints derived via get_current_angle (the same function GRBL uses
    // internally). XYZ via angle_to_coord. coord_arm2effect applies the
    // front/height offsets to give end-effector coordinates.

    float angle_l = 0, angle_r = 0, angle_b = 0;
    get_current_angle( (float)sys.position[X_AXIS],
                       (float)sys.position[Y_AXIS],
                       (float)sys.position[Z_AXIS],
    ...
    const float angle_e = servo_get_angle();

    char x_str[20], y_str[20], z_str[20], e_str[20];
    char jb_str[20], jl_str[20], jr_str[20];

    dtostrf( x,       5, 4, x_str );
    dtostrf( y,       5, 4, y_str );
    dtostrf( z,       5, 4, z_str );
    dtostrf( angle_e, 5, 4, e_str );
    dtostrf( angle_b, 5, 4, jb_str );
    dtostrf( angle_l, 5, 4, jl_str );
    dtostrf( angle_r, 5, 4, jr_str );

    uart_printf( "@3 X%s Y%s Z%s R%s B%s L%s AR%s\n",
                 x_str, y_str, z_str, e_str, jb_str, jl_str, jr_str );
}
